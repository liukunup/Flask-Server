...
def signature_calc(access_key, secret_key, params, headers, body):
    ...
    """
    content_list = list()
    content_list.extend([dat[1] for dat in sorted(params.items(), key=lambda kv:kv[0])])
    content_list.extend([dat[1] for dat in sorted(headers.items(), key=lambda kv: kv[0])])
...
def timestamp_verify(timestamp, period=10, format="%Y-%m-%d %H:%M:%S %Z %z"):
    """
    时间戳验证有效期
    ...
    """
    try:
        target = datetime.datetime.strptime(timestamp, format)
